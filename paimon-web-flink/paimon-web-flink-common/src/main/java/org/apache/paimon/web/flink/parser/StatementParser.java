...
/** Class for parsing and categorizing SQL statements. */
public class StatementParser {
    ...
    public static Map<SqlCategory, List<String>> categorizeStatements(String[] statements) {
        Map<SqlCategory, List<String>> categorizedStatements = new HashMap<>();
        for (String statement : statements) {
            FlinkSqlOperationType operationType = FlinkSqlOperationType.getOperationType(statement);
            SqlCategory category = operationType.getCategory();
            categorizedStatements.computeIfAbsent(category, k -> new ArrayList<>()).add(statement);
        }
