...
public class CdcMangeServiceImpl extends ServiceImpl<CdcManageMapper, CdcManage>
    ...
    public CdcManage run(Long id) {
        ...
        Assert.isFalse(cdcManage.getSubmitStatus().equals(SubmitStatus.PROCESSING), "mission is processing");
        String[] actionParams = strToArgs(cdcManage.getRunArgs());
        Optional<Action> action = Action.Factory.create(actionParams);
        Assert.isTrue(action.isPresent(), "action is not exists,Please check the parameters and troubleshoot through the system log");
        ...
        if (cdcManage.getType().equals(FlinkType.YARN_APPLICATION)) {
            ...
            Configuration configuration = GlobalConfiguration.loadConfiguration(flinkEnv.getFlinkConfPath());
...
//            jarClassLoader.addJar(new File("D:/soft/develop/flink/flink-1.16.2/lib"));
            ...
            Class<?> clazz =
                    null;
            try {
                clazz = jarClassLoader.loadClass(
                        "org.apache.paimon.flink.gateway.yarn.YarnApplicationGateway");
            } catch (ClassNotFoundException e) {
                throw new RuntimeException(e);
            }
            FlinkActionParam flinkActionParam = FlinkActionParam.builder()
    ...
    public CdcManage stop(Long id) {
        ...
        if (cdcManage.getMissionStatus().equals(SubmitStatus.RUNNING)) {
            FlinkActionParam flinkActionParam = FlinkActionParam.builder()
                    .hadoopConfDir(flinkEnv.getHadoopConfPath())
                    .configuration(GlobalConfiguration.loadConfiguration(flinkEnv.getFlinkConfPath()))
                    .flinkConfDir(flinkEnv.getFlinkConfPath())
                    .build();
            boolean isKill = new YarnApplicationGateway(flinkActionParam).killCluster(cdcManage.getApplicationId());
    ...
    private static String[] strToArgs(String args) {
        return StrUtil.split(args, "\n").stream().map(String::trim).flatMap(x -> Stream.of(StrUtil.splitToArray(x, " "))).toArray(String[]::new);
    }
}
